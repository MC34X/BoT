# -*- coding: utf-8 -*-
from lea.ttypes import Message, Location
...
class Talk(object):
    ...
    @loggedIn
    def tempsticker(self,gid,token,aplikasi,link,detail):
        data = {"access_token": token,"line_application": aplikasi,"chat_id": gid,"messages": {"type": "template","altText": "Sticker","template": {"type": "image_carousel","columns": [{"imageUrl": link,"action": {"type": "uri","uri": detail,"area": {"x": 520,"y": 0,"width": 520,"height": 1040}}}]}}}
        self.template(token,data)

    @loggedIn
    def template(self,token,data):
        url = "https://noxtapi.inth.red/bounzen"
        headers = {
